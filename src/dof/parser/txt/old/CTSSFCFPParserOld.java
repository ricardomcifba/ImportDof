...
public class CTSSFCFPParserOld extends Parser {
	...
	public CTSSFCFPParserOld(Connection connection, String fileName, String[] args) {
		super(connection, fileName, args);
		rowStartOffset = -1;
		throw new RuntimeException("Nome do arquivo: " + fileName);
	...
	private void importRow() {
		...
		try {
			...
			sql = "insert into dof_ss (numero, codigo, solicitacao, situacao, matultimamov, ultimamov, matfuncionario, "
					...
					+ "meiosolicitacao, motivonexec, prazo, valor, observacao) "
					...
					+ "%matricula, %meiosolicitacao, %motivonexec, %prazo, %valor, %observacao);";
			sql = assignParametersSS(sql);
			s.execute(sql);
